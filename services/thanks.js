...
function createThanks(teamid, frommemberid, tomemberid, message) {
	if (!tomemberid || !frommemberid || !teamid || !message) {
		throw "All parameters are required";
	}
	if (frommemberid.toString() == tomemberid.toString()) {
		throw "Cannot thank yourself: "+frommemberid;
	}
	var thankspost = new ThanksPost({
		team: teamid,
	...
	return thankspost.save()
		.then(thankspost => {
			...
			return Promise.all([
				teams.incrementSentCount(frommemberid),
				teams.incrementReceivedCount(tomemberid)
			]);
		}).then(results => {
			return thankspost;
		});
...
function sendToTeam(thanksid) {
	return ThanksPost.findById(thanksid)
		.populate({
			path: "createdBy"
		})
		.populate({
		...
		.then(thankspost => {
			...
			var subject = `Thanks to ${thankspost.to.name}`;
			var body = `Thanks To: ${thankspost.to.name}\nFor: ${thankspost.for}\nFrom: ${thankspost.from.name}`;
			return teams.notifyTeam(thankspost.team._id, subject, body);
		}).catch(err => {
			console.log(err);
		});
}
...
function createIdea(teamid, bymemberid, message) {
	...
	return idea.save()
		.then(idea => {
			return teams.incrementIdeaCount(bymemberid);
		}).then(member => {
			return idea;
		});
};
...
async function approveBounty(postid, bountyid) {
	var post = await ThanksPost.findByIdAndUpdate({
		_id: postid,
		posttype: 'idea'
	}, {
		$push: {
	...
	var bounty = await teams.getBounty(bountyid);
